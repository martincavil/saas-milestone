...
export function LoginForm({ error, message }: Props) {
  ...
  function reset() { setErr(''); setSuccess('') }
  ...
  async function handleSignIn(e: React.FormEvent) {
    ...
    if (authErr) {
      setErr(authErr.message === 'Invalid login credentials'
  ...
  if (success) {
    return (
      <div className="rounded-2xl border p-8 text-center" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
        ...
        <p className="font-semibold font-poppins mb-1" style={{ color: 'var(--text)' }}>{success}</p>
        <button onClick={() => setSuccess('')} className="mt-4 text-xs text-indigo-500 hover:text-indigo-600 transition-colors">
          Back to sign in
        </button>
      </div>
    )
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--surface)', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
      ...
      <div className="flex border-b" style={{ borderColor: 'var(--border)' }}>
        {(['signin', 'signup'] as Tab[]).map(t => (
          <button
            ...
            style={{
              color:       tab === t ? '#6366f1' : 'var(--text-2)',
              background:  tab === t ? 'var(--surface)' : 'var(--bg-2)',
      ...
      <div className="p-6 space-y-4">
        ...
        <button
          ...
          disabled={gLoading}
          className="flex w-full items-center justify-center gap-3 rounded-xl border py-2.5 text-sm font-medium transition-all hover:shadow-sm disabled:opacity-50"
          style={{ borderColor: 'var(--border)', color: 'var(--text)', background: 'var(--surface)' }}
          onMouseEnter={e => (e.currentTarget.style.background = 'var(--bg-2)')}
          onMouseLeave={e => (e.currentTarget.style.background = 'var(--surface)')}
        >
          {gLoading ? <Loader2 size={16} className="animate-spin text-gray-400" /> : <GoogleIcon />}
        ...
        <form onSubmit={tab === 'signin' ? handleSignIn : handleSignUp} className="space-y-3">
          ...
          <div>
            ...
            {tab === 'signup' && password.length > 0 && password.length < 8 && (
              <p className="mt-1 text-xs text-red-500">{8 - password.length} more character{8 - password.length > 1 ? 's' : ''} needed</p>
            )}
          </div>
        ...
        {tab === 'signup' && (
          <p className="text-center text-xs" style={{ color: 'var(--text-3)' }}>
            ...
            <a href="#" className="text-indigo-500 hover:text-indigo-600 transition-colors">Terms</a>
            {' '}and{' '}
