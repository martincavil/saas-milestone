...
import { CreditCard, Pencil, Trash2, Loader2, AlertCircle, Plus, ChevronDown, ChevronUp } from 'lucide-react'
...
function AddForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 pt-3 border-t border-white/8">
      <div>
      ...
      <div className="flex gap-2">
        ...
        >
          {loading ? <Loader2 size={14} className="animate-spin" /> : (connectionId ? 'Save' : 'Connect')}
        </button>
        <button
          type="button"
          onClick={onCancel}
          ...
          {editingId === conn.id ? (
            ...
            <div className="flex items-center justify-between rounded-xl border border-white/8 bg-white/3 px-3.5 py-2.5">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-xs font-medium text-white">{conn.stripe_account_name}</p>
