...
export async function POST() {
  return withAuth(async (user, service) => {
    ...
    if (!customerId) {
      const customers = await getStripe().customers.list({ email: user.email!, limit: 5 })
    ...
    const subscriptions = await getStripe().subscriptions.list({ customer: customerId, status: 'active', limit: 1 })
