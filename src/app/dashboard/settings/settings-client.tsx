...
import { Loader2, Zap, CreditCard, Check, ExternalLink, RefreshCw } from 'lucide-react'
...
export function SettingsClient({ email, isSubscribed, hasStripeCustomer }: Props) {
  ...
  return (
    <div className="p-6 space-y-8 max-w-lg">
      <div>
        <h1 className="text-xl font-bold text-white" style={{ fontFamily: 'var(--font-poppins)' }}>Settings</h1>
      ...
      {/* Plan */}
      <section className="space-y-3">
        <p className="text-xs font-medium text-white/35 uppercase tracking-wider">Plan</p>
        ...
        {isSubscribed ? (
          <div className="rounded-2xl border border-indigo-500/25 bg-indigo-500/8 p-5">
            ...
            <ul className="space-y-1.5 mb-4">
              {['Full analytics dashboard', 'Multi-SaaS overview', 'Manual posting', 'Priority support'].map(f => (
                <li key={f} className="flex items-center gap-2 text-xs text-white/55">
                  <Check size={11} className="text-indigo-400 flex-shrink-0" strokeWidth={2.5} />
                  {f}
                </li>
              ))}
            </ul>
          ...
          <div className="rounded-2xl border border-white/8 bg-white/4 p-5">
            ...
            <div className="rounded-xl border border-indigo-500/20 bg-indigo-500/8 p-4 mb-4">
              <p className="text-xs font-semibold text-indigo-300 mb-2">Upgrade to Pro — $9/month</p>
              <ul className="space-y-1.5 mb-3">
                {['Full analytics dashboard', 'Multi-SaaS MRR breakdown', 'Manual milestone posting', 'Priority support'].map(f => (
              ...
              >
                {upgradeLoading ? <Loader2 size={14} className="animate-spin" /> : <Zap size={14} fill="white" />}
                Upgrade now
              </button>
            </div>
            <p className="text-xs text-white/25 text-center">Cancel anytime · No questions asked</p>
          </div>
        )}
      </section>
      ...
      {/* Sync subscription */}
      <section className="space-y-3">
        <p className="text-xs font-medium text-white/35 uppercase tracking-wider">Troubleshooting</p>
        <div className="rounded-2xl border border-white/8 bg-white/4 p-4">
          <p className="text-xs font-medium text-white/70 mb-1">Already paid but not on Pro?</p>
          <p className="text-xs text-white/35 mb-3">
            Webhooks can fail in local or staging environments. This will check Stripe directly and activate your account.
          </p>
          <button
            onClick={handleSync}
          ...
          {syncResult && (
            <p className={`mt-2 text-xs ${syncResult.startsWith('✓') ? 'text-green-400' : 'text-amber-400'}`}>
              {syncResult}
            </p>
          )}
        </div>
      ...
      <section className="space-y-2">
        <p className="text-xs font-medium text-white/35 uppercase tracking-wider">Links</p>
