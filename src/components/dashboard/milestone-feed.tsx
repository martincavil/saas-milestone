    ...
    return (
      <div className="rounded-2xl border border-dashed border-white/10 p-10 text-center">
        ...
        <p className="mt-1 text-xs text-white/20">
          We check your metrics every hour.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-2">
      {milestones.map((m) => {
        ...
        return (
          <div
            key={m.id}
            className="flex items-center gap-4 rounded-xl border border-white/8 bg-white/4 px-4 py-3 hover:bg-white/6 transition-colors"
          >
            <div
              className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${CATEGORY_BG[cat] ?? "bg-white/10"}`}
            >
              {CATEGORY_ICONS[cat] ?? (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <p className="font-semibold text-white text-sm font-poppins">
                  {label}
                </p>
