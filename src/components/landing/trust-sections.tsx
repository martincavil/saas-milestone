import { Shield, Key, RefreshCw, Eye, ChevronDown, TrendingUp, Clock, Image, Send, Zap, Users, Timer, DollarSign } from 'lucide-react'
...
export function StatsBar() {
  const stats = [
    { icon: <TrendingUp size={16} className="text-indigo-500" />, value: '8', label: 'milestones tracked' },
    { icon: <Timer size={16} className="text-indigo-500" />, value: '< 60 min', label: 'detection time' },
    { icon: <Image size={16} className="text-indigo-500" />, value: '1200×630', label: 'card resolution' },
    { icon: <DollarSign size={16} className="text-indigo-500" />, value: '$0', label: 'until $100 MRR' },
  ]
  return (
          ...
          {stats.map((s) => (
            <div key={s.label} className="flex flex-col items-center gap-1 text-center">
              <div className="flex items-center gap-1.5">
                {s.icon}
                <span className="text-xl font-extrabold text-gray-900" style={{ fontFamily: 'var(--font-poppins)' }}>
                  {s.value}
                </span>
              </div>
...
const TWEETS = [
  {
    ...
    handle: '@thomasbuilds',
    avatar: '#6366f1',
    content: "I hit $500 MRR last night and woke up to 12 notifications. saas-milestone had already posted the card while I was asleep. That's the dream.",
    time: '2h ago',
  },
  {
    name: 'Sara L.',
    handle: '@saraships',
    avatar: '#0891b2',
    content: "Finally stopped manually screenshotting Stripe and posting to X every time I hit a new milestone. This tool does it automatically. Obsessed.",
    time: '1d ago',
  },
  {
    name: 'Kevin M.',
    ...
    avatar: '#059669',
    content: "The setup took literally 2 minutes. Paste the Stripe key, connect Twitter, done. My $100 MRR tweet posted 45 minutes after I hit the milestone.",
    time: '3d ago',
  },
]
  ...
  return (
    <section className="border-t border-gray-100 py-24">
      <div className="mx-auto max-w-5xl px-5">
        ...
        <ScrollRevealGrid className="grid gap-4 md:grid-cols-3" staggerDelay={0.12}>
          {TWEETS.map((t) => (
            <div
              key={t.handle}
            ...
            >
              <div className="mb-4 flex items-center justify-between">
                ...
                {/* X logo */}
                <svg viewBox="0 0 24 24" className="h-4 w-4 fill-current text-gray-300">
        ...
        <div className="grid items-center gap-16 lg:grid-cols-2">
          <ScrollReveal direction="left">
            ...
            <h2 className="mb-5 text-4xl font-bold text-gray-900" style={{ fontFamily: 'var(--font-poppins)' }}>
              Building in public
              <br />is a growth channel
            </h2>
            <p className="mb-6 text-gray-500 leading-relaxed">
              Every milestone post is a signal to your audience that you're for real.
              It brings in early adopters, builds trust, and creates a public track record
              that no ad can buy.
            </p>
            <ul className="space-y-4">
              {[
                { title: 'Community momentum', desc: '#buildinpublic posts routinely get 10–100x more reach than regular product updates.' },
                { title: 'Founder credibility', desc: "Numbers don't lie. Sharing MRR milestones publicly builds the kind of trust that converts followers into customers." },
                { title: 'Accountability loop', desc: 'Public milestones keep you shipping. Visibility creates pressure — in the best possible way.' },
              ].map((item) => (
          ...
          <ScrollReveal direction="right" delay={0.1}>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={item.amount} className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-900">
                          Just hit <span className="text-indigo-600">{item.amount}</span>
                        </p>
                        <p className="text-xs text-gray-400">#buildinpublic · posted automatically</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-xs font-semibold text-gray-700">{item.likes} ♥</p>
                      <p className="text-xs text-gray-400">{item.reach}</p>
                    </div>
                  </div>
                </div>
              ))}
              <p className="text-center text-xs text-gray-400 pt-1">Higher milestones → more reach → more inbound</p>
            </div>
          </ScrollReveal>
        </div>
      </div>
        ...
        <ScrollReveal>
          <div className="rounded-2xl border border-indigo-100 bg-indigo-50/50 p-8">
            <div className="mb-6 flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-indigo-600 text-lg font-bold text-white" style={{ fontFamily: 'var(--font-poppins)' }}>
                M
              </div>
              <div>
            ...
            <blockquote className="text-gray-700 leading-relaxed">
              "I kept manually screenshotting Stripe every time I hit a milestone and
              posting to Twitter. It took 10 minutes I didn't want to spend. Then I'd
              forget, and the moment passed. I built this so the celebration happens
              automatically — while you stay focused on what matters."
            </blockquote>
            <p className="mt-4 text-sm text-gray-500">
              It takes 2 minutes to set up. After that, you never have to think about it again.
            </p>
          </div>
        </ScrollReveal>
      </div>
        ...
        <div className="grid items-center gap-16 lg:grid-cols-2">
          <ScrollReveal direction="left">
            ...
            <p className="mb-8 text-gray-500 leading-relaxed">
              We only ask for a <strong className="text-gray-700">restricted read-only key</strong> —
              the kind Stripe was built for. We can see your subscriptions.
              We cannot move money, create charges, or access anything else.
            </p>
            ...
            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-6">
              ...
              <div className="space-y-2">
                {[
                  ...
                  { resource: 'Payment methods', permission: 'No access', ok: false },
                  ...
                  <div
                    ...
                    style={{
                      borderColor: row.ok ? 'rgba(99,102,241,0.2)' : 'rgba(0,0,0,0.06)',
                      background: row.ok ? 'rgba(99,102,241,0.04)' : '#fff',
                    }}
                  >
                    <span className={row.ok ? 'font-medium text-gray-800' : 'text-gray-400'}>
                      {row.resource}
                    </span>
                    <span
                      className="rounded-full px-2.5 py-0.5 text-xs font-medium"
                      style={{
                        background: row.ok ? 'rgba(99,102,241,0.12)' : 'rgba(0,0,0,0.04)',
  ...
  const steps = [
    {
      ...
      detail: 'Create a Stripe restricted key with read access to subscriptions. Takes 30 seconds. Paste it in the dashboard.',
    },
    {
      icon: <Clock size={20} className="text-indigo-500" />,
      step: '02',
      title: 'Vercel cron runs every hour',
      detail: 'A server-side cron job pulls your active subscriptions via the Stripe API, normalizes them to monthly amounts, and computes your MRR.',
    },
    {
      icon: <TrendingUp size={20} className="text-indigo-500" />,
      step: '03',
      title: 'Threshold detection',
      detail: 'We compare the new MRR against your last recorded value. If you crossed $1, $10, $50, $100, $500, $1k, $5k, or $10k — we flag it.',
    },
    {
      icon: <Image size={20} className="text-indigo-500" />,
      step: '04',
      title: 'Card generated with Satori',
      detail: 'A 1200×630 PNG is rendered server-side — your SaaS name, the milestone amount, and a progress bar toward the next threshold.',
    },
    {
      icon: <Send size={20} className="text-indigo-500" />,
      step: '05',
      title: 'Posted to X automatically',
      detail: 'The card uploads to Twitter as media, then posts with a short caption and #buildinpublic hashtags. Done — while you were sleeping.',
    },
  ]
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  <span className="text-xs font-mono font-bold text-gray-300">{s.step}</span>
...
const FAQS = [
  {
    ...
    a: "Yes. We ask for a restricted key with read-only access to subscriptions — no write permissions. Stripe enforces this at the API level. You can revoke it instantly from your Stripe dashboard at any time.",
  },
  {
    q: 'What exactly counts toward MRR?',
    a: "Active subscriptions only. Monthly plans count as-is. Annual plans are divided by 12. Weekly plans are multiplied by (52/12). We only count USD subscriptions for now.",
  },
  {
    q: "What if I already crossed some milestones before signing up?",
    a: "We record your current MRR when you connect. From that point forward, if you cross a new threshold (that's higher than your current MRR), we detect it and post. We won't retroactively post milestones you hit before connecting.",
  },
  {
  ...
  {
    q: "Can I preview or edit the tweet before it posts?",
