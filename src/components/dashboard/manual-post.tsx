...
import {
  TrendingUp, Users, Globe, Star, Mail, Send,
  Loader2, ExternalLink, AlertCircle, CheckCircle2,
  Lock,
} from 'lucide-react'
import { MILESTONE_CONFIGS, CATEGORIES_ORDER, type MilestoneCategory } from '@/types'
...
const CATEGORY_COLORS: Record<MilestoneCategory, { bg: string; text: string; border: string }> = {
  mrr:         { bg: 'rgba(99,102,241,0.15)',  text: '#a5b4fc', border: 'rgba(99,102,241,0.3)' },
  followers:   { bg: 'rgba(14,165,233,0.15)',  text: '#7dd3fc', border: 'rgba(14,165,233,0.3)' },
  users:       { bg: 'rgba(139,92,246,0.15)',  text: '#c4b5fd', border: 'rgba(139,92,246,0.3)' },
  visits:      { bg: 'rgba(16,185,129,0.15)',  text: '#6ee7b7', border: 'rgba(16,185,129,0.3)' },
  stars:       { bg: 'rgba(245,158,11,0.15)',  text: '#fcd34d', border: 'rgba(245,158,11,0.3)' },
  subscribers: { bg: 'rgba(244,63,94,0.15)',   text: '#fda4af', border: 'rgba(244,63,94,0.3)' },
}
...
export function ManualPost({ isSubscribed, saasName, currentMRR, twitterConnected }: ManualPostProps) {
  ...
  const milestone  = [...config.thresholds].reverse().find(t => parsed >= t) ?? null
  ...
  if (!twitterConnected) {
    return (
      <div className="rounded-2xl border border-white/8 bg-white/3 p-5">
        <div className="flex items-start gap-3">
          <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl bg-sky-500/15">
            <XIcon size={15} />
          </div>
          <div>
            <p className="text-sm font-semibold text-white" style={{ fontFamily: 'var(--font-poppins)' }}>
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-white/4 p-5 space-y-5">
      ...
      <div className="grid grid-cols-3 gap-2">
        {CATEGORIES_ORDER.map(cat => {
          ...
          return (
            <button
              key={cat}
              onClick={() => { setCategory(cat); setRawValue(''); setError(null); setResult(null) }}
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            inputMode="numeric"
            value={rawValue}
            onChange={e => { setRawValue(e.target.value); setError(null); setResult(null) }}
            placeholder={config.thresholds[Math.floor(config.thresholds.length / 2)].toString()}
            ...
            style={{ paddingLeft: config.unit ? '28px' : '14px', paddingRight: '14px' }}
          />
        </div>
      </div>
        ...
        >
          {milestone ? (
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-white/40 mb-0.5">Milestone to post</p>
                <p className="text-lg font-extrabold" style={{ color: colors.text, fontFamily: 'var(--font-poppins)' }}>
                  {config.formatValue(milestone)}
                  <span className="text-sm font-medium text-white/40 ml-1">{config.suffix}</span>
                </p>
              </div>
              <div className="text-right">
                <p className="text-xs text-white/30">You're at</p>
                <p className="text-sm font-semibold text-white/60">
                  {config.unit}{parsed.toLocaleString()}{config.suffix}
                </p>
              </div>
            </div>
          ) : (
            <p className="text-xs text-white/35">
              No milestone threshold reached yet.
              First threshold: {config.unit}{config.thresholds[0]}{config.suffix}
            </p>
          )}
        </div>
      )}
      ...
      <button
        ...
        style={{
          background: milestone ? `linear-gradient(135deg, ${colors.bg}, rgba(255,255,255,0.06))` : 'rgba(255,255,255,0.04)',
      ...
      {!milestone && parsed === 0 && (
        <div className="flex flex-wrap gap-1.5">
          {config.thresholds.slice(0, 5).map(t => (
            ...
            >
              {config.unit}{t >= 1000 ? `${t/1000}k` : t}{config.suffix}
            </button>
          ))}
          <span className="px-1 text-xs text-white/20 self-center">← click to fill</span>
        </div>
      )}
    </div>
  )
