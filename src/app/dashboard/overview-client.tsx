...
import { MILESTONE_CONFIGS, type MilestoneCategory, type MilestoneHit } from '@/types'
import { TrendingUp, Users, Globe, Star, Mail, Trophy, Zap, Settings2, X, RefreshCw } from 'lucide-react'
...
const WIDGET_KEYS = ['mrr', 'accounts', 'milestones', 'categories', 'recent', 'social'] as const
...
export function OverviewClient({ milestones, twitterAccounts, stripeAccounts }: Props) {
  ...
  useEffect(() => {
    fetch('/api/mrr').then(r => r.json()).then(d => { setMrrData(d); setMrrLoading(false) })
  }, [])
  ...
  const postingRate   = milestones.length > 0 ? Math.round((totalPosted / milestones.length) * 100) : 0
  const thisMonth     = milestones.filter(m => new Date(m.hit_at) > new Date(Date.now() - 30*24*3600*1000)).length
  const byCategory    = Object.entries(
    milestones.reduce<Record<string, number>>((a, m) => { const c = m.category ?? 'mrr'; a[c]=(a[c]??0)+1; return a }, {})
  ).sort((a,b) => b[1]-a[1])
  ...
  return (
    <div className="min-h-full p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white" style={{ fontFamily: 'var(--font-poppins)' }}>Overview</h1>
          <p className="text-xs text-white/35 mt-0.5">All your metrics in one place</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => { setMrrLoading(true); fetch('/api/mrr').then(r=>r.json()).then(d=>{setMrrData(d);setMrrLoading(false)}) }}
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs transition-colors ${
              configOpen ? 'border-indigo-500/40 bg-indigo-500/15 text-indigo-300' : 'border-white/10 text-white/40 hover:bg-white/8 hover:text-white/70'
            }`}
          >
            <Settings2 size={13} />
            Customize
      ...
      {configOpen && (
        <div className="rounded-2xl border border-white/10 bg-white/4 p-4">
          <p className="text-xs font-semibold text-white/60 mb-3">Show / hide widgets</p>
      ...
      {show('mrr') && (
        <section>
          <p className="text-xs font-medium text-white/30 uppercase tracking-wider mb-3">Revenue</p>
          {mrrLoading ? (
            <div className="rounded-2xl border border-white/8 bg-white/4 p-5 text-xs text-white/30">Loading…</div>
          ) : mrrData?.connected ? (
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              <StatCard
                ...
                color="#6366f1"
                sparkData={[mrrData.totalMRR*0.3, mrrData.totalMRR*0.5, mrrData.totalMRR*0.65, mrrData.totalMRR*0.8, mrrData.totalMRR*0.9, mrrData.totalMRR]}
              />
              <StatCard
                label="ARR"
              ...
              <StatCard
                ...
                value={mrrData.totalSubscribers > 0 ? fmt(Math.round(mrrData.totalMRR / mrrData.totalSubscribers)) : '—'}
                sub="MRR per user"
                color="#10b981"
              />
            </div>
            ...
            <div className="rounded-2xl border border-dashed border-white/12 p-8 text-center">
              <p className="text-sm text-white/35">Connect Stripe in the Connectors tab.</p>
            </div>
          )}
        </section>
      )}
      ...
      {show('accounts') && mrrData?.accounts?.length > 0 && (
        <section>
          <p className="text-xs font-medium text-white/30 uppercase tracking-wider mb-3">SaaS breakdown</p>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                {mrrData.accounts.map((a: any) => (
                  <div key={a.id} className="rounded-xl border border-white/8 bg-white/3 px-3 py-2">
                    ...
                    <p className="text-sm font-bold text-white" style={{ fontFamily: 'var(--font-poppins)' }}>{fmt(a.mrr)}</p>
                    <p className="text-xs text-white/30">{mrrData.totalMRR > 0 ? Math.round((a.mrr/mrrData.totalMRR)*100) : 0}% of total</p>
                  </div>
                ))}
              </div>
            )}
      ...
      {show('milestones') && (
        <section>
          <p className="text-xs font-medium text-white/30 uppercase tracking-wider mb-3">Milestones</p>
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <StatCard label="Total hit" value={milestones.length.toString()} sub="All time" color="#f59e0b" />
            <StatCard label="This month" value={thisMonth.toString()} sub="Last 30 days" color="#f59e0b" />
            <StatCard label="Auto-posted" value={totalPosted.toString()} sub="Posted to X" color="#0ea5e9" />
            <StatCard label="Posting rate" value={`${postingRate}%`} sub={`${totalPosted}/${milestones.length}`} color="#10b981" />
          </div>
        </section>
      )}
      ...
      {show('categories') && byCategory.length > 0 && (
        <section>
          <p className="text-xs font-medium text-white/30 uppercase tracking-wider mb-3">By category</p>
          <div className="rounded-2xl border border-white/8 bg-white/4 p-5">
            <HorizontalBars
              data={byCategory.map(([cat, count]) => {
                ...
                return { label: config?.label ?? cat, value: count, color: colorMap[cat] }
              })}
            />
          </div>
        </section>
      ...
      {show('social') && (
        <section>
          <p className="text-xs font-medium text-white/30 uppercase tracking-wider mb-3">Connected social accounts</p>
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">
            {/* X accounts */}
            {twitterAccounts.length > 0 ? twitterAccounts.map(acc => (
              <div key={acc.twitter_user_id} className="rounded-2xl border border-sky-500/20 bg-sky-500/8 p-4">
                <div className="flex items-center gap-2 mb-2">
                  <div className="flex h-7 w-7 items-center justify-center rounded-full bg-sky-500/20 text-sky-400">
                    <XIcon size={13} />
                  </div>
                  <p className="text-xs font-semibold text-sky-300">X (Twitter)</p>
                </div>
                <p className="text-sm font-bold text-white">@{acc.screen_name}</p>
                <p className="text-xs text-white/30 mt-0.5">Auto-posts enabled</p>
              </div>
            )) : (
      ...
      {show('recent') && milestones.length > 0 && (
        <section>
          <p className="text-xs font-medium text-white/30 uppercase tracking-wider mb-3">Recent activity</p>
          <div className="rounded-2xl border border-white/8 bg-white/4 overflow-hidden">
            {milestones.slice(0, 8).map((m, i) => {
              ...
              const label  = config ? `${config.formatValue(m.amount)} ${config.label}` : `$${m.amount}`
              const catColors: Record<MilestoneCategory, string> = {
                mrr: 'text-indigo-400', followers: 'text-sky-400', users: 'text-violet-400',
                visits: 'text-emerald-400', stars: 'text-amber-400', subscribers: 'text-rose-400',
              }
              return (
                <div
                  key={m.id}
                ...
                >
                  <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-white/6">
                    <Trophy size={12} className={catColors[cat] ?? 'text-white/40'} />
                  </div>
      ...
      {milestones.length === 0 && !mrrLoading && (
        <div className="rounded-2xl border border-dashed border-white/10 p-12 text-center">
          ...
          <p className="text-sm text-white/35">No milestones yet. Connect Stripe to start tracking.</p>
        </div>
      )}
    </div>
  )
